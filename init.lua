...
require('lazy').setup({
  ...
  -- LSP Plugins
  {
    ...
    config = function()
      ...
      -- Infer executable name from an lspconfig file.
      -- For table cmd, returns cmd[1]. For function cmd, extracts the first `local cmd = '...'`
      -- literal from the source (the default before any project-local override).
      -- Falls back to the Mason package name or server name if neither is found.
      local lsp_exe_cache = {}
      local function get_lsp_exe(server_name)
        if lsp_exe_cache[server_name] == nil then
          local path = vim.fn.stdpath 'data' .. '/lazy/nvim-lspconfig/lsp/' .. server_name .. '.lua'
          local f = io.open(path, 'r')
          local exe = false
          if f then
            local src = f:read '*a'
            f:close()
            -- table cmd: cmd = { 'executable', ... }
            exe = src:match "cmd%s*=%s*{%s*'([^']+)'"
              -- function cmd: local cmd = 'executable'
              or src:match "local cmd = '([^']+)'"
              or false
          end
          lsp_exe_cache[server_name] = exe
        end
        return lsp_exe_cache[server_name]
      end

      -- Check if tool should be installed via Mason (true = install, false = skip)
      local function should_install(server_name)
        local executable_name = get_lsp_exe(server_name) or (lspconfig_to_package[server_name] or server_name)
        ...
        if debug_lsp_install then
          local status = exe_path == '' and 'NOT FOUND (will install)'
            or (is_system_tool and string.format('FOUND: %s (skip install)', exe_path) or 'Only in Mason (keep)')
      ...
      local additional_tools = { 'stylua', 'markdownlint', 'oxfmt', 'gofumpt' }
      ...
      for server_name, server_config in pairs(servers or {}) do
        server_config.condition = nil -- Remove condition field (not part of LSP config)
        -- vim.lsp.config merges with lspconfig defaults (lsp/*.lua) automatically
